...
const validatePagamentoInput = (pagamento) => {
  ...
  for (const field of requiredFields) {
    if (!(field in pagamento) || pagamento[field] === null || pagamento[field] === undefined) {
...
const validatePagamentoUpdateInput = (updateData) => {
  ...
  if (updateData.data !== undefined) {
    if (typeof updateData.data !== 'string' || !validateDate(updateData.data)) {
      errors.push('data deve estar no formato DD/MM/YYYY');
    }
  }
...
export { validatePagamentoInput, validatePagamentoUpdateInput };
