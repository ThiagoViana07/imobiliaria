...
const validateClientInput = (client) => {
  ...
  if (
    client.rg &&
    (typeof client.rg !== "string" || client.rg.trim().length === 0)
  ) {
  ...
  if (
    ...
    (typeof client.data_nascimento !== "string" ||
      !validateDate(client.data_nascimento))
  ) {
  ...
  if (
    client.profissao &&
    (typeof client.profissao !== "string" ||
      client.profissao.trim().length === 0)
  ) {
  ...
  if (
    client.estado_civil &&
    (typeof client.estado_civil !== "string" ||
      client.estado_civil.trim().length === 0)
  ) {
  ...
  if (!Array.isArray(client.telefone) || client.telefone.length === 0) {
    ...
    client.telefone.forEach((tel, index) => {
      if (typeof tel !== "object" || tel === null) {
        ...
        if (
          tel.residencial &&
          (typeof tel.residencial !== "string" ||
            !validatePhone(tel.residencial))
        ) {
          errors.push(
            `Telefone[${index}].residencial deve estar no formato (XX) XXXXX-XXXX`,
          );
        }
        if (
          tel.comercial &&
          ...
          errors.push(
            `Telefone[${index}].comercial deve estar no formato (XX) XXXXX-XXXX`,
          );
        }
        if (!tel.residencial && !tel.comercial) {
          errors.push(
            `Telefone[${index}] deve ter pelo menos residencial ou comercial`,
          );
        }
      }
    });
      ...
      if (typeof end !== "object" || end === null) {
        ...
        enderecoFields.forEach((field) => {
          if (
            ...
            typeof end[field] !== "string" ||
            end[field].trim().length === 0
          ) {
            errors.push(
              `Endereço[${index}].${field} deve ser uma string não vazia`,
            );
          }
        });
        ...
        if (
          ...
          errors.push(
            `Endereço[${index}].complemento deve ser uma string ou null`,
          );
        }
      }
    });
...
const validateClientUpdateInput = (updateData) => {
  ...
  if (updateData.nome !== undefined) {
    if (
      typeof updateData.nome !== "string" ||
      updateData.nome.trim().length === 0
    ) {
      errors.push("Nome deve ser uma string não vazia");
    }
  }
  ...
  if (updateData.rg !== undefined) {
    if (
      typeof updateData.rg !== "string" ||
      updateData.rg.trim().length === 0
    ) {
      errors.push("RG deve ser uma string não vazia");
    }
  }
  ...
  if (updateData.profissao !== undefined) {
    if (
      typeof updateData.profissao !== "string" ||
      updateData.profissao.trim().length === 0
    ) {
      errors.push("Profissão deve ser uma string não vazia");
    }
  }
  ...
  if (updateData.telefone !== undefined) {
    if (
      !Array.isArray(updateData.telefone) ||
      updateData.telefone.length === 0
    ) {
      errors.push("Telefone deve ser um array não vazio");
    } else {
  ...
  if (updateData.endereco !== undefined) {
    if (
      !Array.isArray(updateData.endereco) ||
      updateData.endereco.length === 0
    ) {
      errors.push("Endereço deve ser um array não vazio");
    } else {
      updateData.endereco.forEach((end, index) => {
        if (typeof end !== "object" || end === null) {
          ...
          if (end.cep && !validateCEP(end.cep)) {
            errors.push(
              `Endereço[${index}].cep deve estar no formato XXXXX-XXX`,
            );
          }
