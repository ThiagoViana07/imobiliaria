...
async function insertPagamento(pagamento) {
  try {
    ...
    await fs.promises.writeFile(caminhoArquivo, JSON.stringify(novaListaPagamentos));
  } catch (err) {
...
async function editPagamento(modificacoes, id) {
  try {
    ...
    await fs.promises.writeFile(caminhoArquivo, JSON.stringify(pagamentos));
  } catch (err) {
...
async function deletePagamento(id) {
  try {
    ...
    if (listaFiltrada.length === pagamentos.length)
...
export { getAllPagamentos, getPagamentoById, insertPagamento, editPagamento, deletePagamento };
